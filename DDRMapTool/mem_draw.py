...
class MemDraw:
    ...
    def _plot_agent(self, agent, agent_index):
        rects = plt.bar(self._ddr_pos[agent.ddr_tag.value],
                        agent.size_m,
                        bottom=agent.start_addr / 1024.0 / 1024.0,
                        facecolor=self._colors[agent_index],
                        edgecolor='None',
                        width=BAR_WIDTH,
                        alpha=0.5)
        rect = rects[0]
        ...
        plt.text(rect.get_x() + rect.get_width(),
                 ...
                 '0x%08X' % agent.end_addr, ha='left', va='top', size=9)
    ...
    def plot(self):
        fig = plt.figure()
        mapping = self._config.place_memory()
        self._config.print_mapping(mapping)
        ...
        for agents in mapping:
            for agent in agents:
                if agent.ddr_op == DDROp.R or agent.unused:
                    continue
        ...
        yticks = range(0, BAR_HEIGHT + 16, 16)
        plt.yticks(yticks, yticks)

        plt.xticks(self._ddr_pos, self._ddr_names)
        plt.title('DDR Memory Usage for %s' % self._config.name)
